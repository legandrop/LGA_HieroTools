...
- **Script utilizado**: `LGA_NKS_Coordination_Panel_py/LGA_NKS_Flow_Thumbs.py` y, para el reemplazo en Flow, `LGA_NKS_Coordination_Panel_py/LGA_NKS_Flow_UpdateThumb.py`
...
- **Script utilizado**: `LGA_NKS_Coordination_Panel_py/LGA_NKS_FileManagerS3_DownloadClip.py`
...
3. **Check Shots Exist** - Chequea si los shots del track comp existen en Flow
...
12. **Reveal in Flow** - `Ctrl+Shift+F` - Abre la task comp en Chrome
...
- `LGA_HieroTools/LGA_NKS_Coordination_Panel.py` - `FlowProdPanel.__init__()` define el orden y color de los botones; `FlowProdPanel.create_buttons()` construye el layout; `create_thumbnail_for_selected_clip()` y `update_thumbnail_in_flow_for_selected_clip()` resuelven los dos gestos de Thumbnail
- `LGA_NKS_Coordination_Panel_py/LGA_NKS_Flow_ShowInFlow.py` - Funcionalidad de Reveal in Flow
- `LGA_HieroTools/LGA_NKS_Coordination_Panel_py/LGA_NKS_Flow_Thumbs.py` - `main()` guarda el snapshot local; `zoom_to_fill_simple()` y `crop_to_aspect_ratio()` preparan la imagen
- `LGA_HieroTools/LGA_NKS_Coordination_Panel_py/LGA_NKS_Flow_UpdateThumb.py` - `update_thumbnail_in_flow()` inicia el reemplazo; `ThumbReplaceDialog` presenta la comparación; `UploadThumbWorker` sube sin bloquear la UI
...
- `LGA_NKS_Coordination_Panel_py/LGA_NKS_FileManagerS3_DownloadClip.py` - Funcionalidad de Download Clip

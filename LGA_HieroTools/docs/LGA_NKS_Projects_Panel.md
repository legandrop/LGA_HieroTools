...
- `LGA_NKS_Projects_Panel_py/LGA_NKS_ProjectMediaPreview.py` - Clase `ProjectMediaPreviewDialog` y `_TimelineCell`: selector de track en la propia tabla y estrategia de inserción con tres botones, sobre un eje temporal continuo con color real de BinItem, playhead visible y proyección roja de la media nueva, sin mutar el timeline.
...
- Colapsar y cerrar proyectos abiertos:
  - Click en el nombre o en el triangulo de un proyecto ABIERTO lo colapsa o expande (solo el panel, no toca Hiero); en uno cerrado lo abre. Antes el click en un abierto volvia a llamar a `openProject`. Lo colapsado vive en `collapsed_projects` del panel y sobrevive al rearmado de la lista, no a reabrir NKS.
  - El triangulo se DIBUJA (`_triangle_pixmap`, cuadrado de `TRIANGLE_SIZE` px): como caracteres de texto, la fuente dibujaba el que apunta a la derecha mucho mas chico que el que apunta hacia abajo. Nombre y triangulo comparten el hover (`ProjectItem.eventFilter`), porque hacen lo mismo.
  - La x de cerrar aparece solo con el mouse sobre la fila de un proyecto abierto, pegada al nombre, y reserva su lugar oculta para que la fila no salte. Usa `Style.BTN_CLOSE` con la cruz dibujada en `Color.TEXT` y caja de 16 px, para que el rojo del hover no sea enorme.
  - `project.close()` desde Python NO pregunta ni guarda: descarta los cambios en silencio (medido con `+Building_Blocks/explore_project_close.py`). Por eso `close_project()` consulta `modifiedSinceLastSave()` y pregunta Cancel / Don't save / Save (`ask_save_discard_cancel` del helper de carteles); si esa llamada falla, pregunta igual. Si era el proyecto del timeline activo, va al ultimo timeline de otro proyecto abierto del panel.
  - El panel escucha `kAfterProjectClose` y se refresca aunque el cierre venga de `File > Close`. El handler se registra una vez por proceso y se guarda en `hiero.core`, para desregistrarlo en cada reimport y no acumular handlers de paneles destruidos.
...
- Durante el switch la ventana principal no repinta (`FREEZE_UI_DURING_SWITCH`, reactivado en un `finally`): el usuario ve solo el estado final. Los snapshots de widgets y la espera de limpieza son diagnosticos y quedan apagados (`SWITCH_DIAGNOSTIC_LOG_WIDGETS`, `SWITCH_DIAGNOSTIC_CLEANUP_WAIT`). Gain/gamma/saturation no se guardan: siguen pasando de un timeline al siguiente.
  - La vista es PERSISTENTE (`logs/ProjectsPanel_TimelineViews.json`, sobrevive a reabrir NKS) y la clave es carpeta del `.hrox` + nombre del proyecto SIN version + secuencia: el `101` de todas las versiones de un proyecto comparte la vista, y studio/client quedan separados porque viven en roots distintos. Antes de escribir se relee el archivo, asi dos NKS abiertos no se pisan las vistas.
  - Solo el ultimo timeline de cada contexto (toggle Studio/Client) dura la sesion: `logs/ProjectsPanel_TimelineMemory_<PID>.json`. Los tres JSON van en `logs/` porque guardan nombres reales de proyectos y esa carpeta no se versiona.
...
- **Drop media:** al arrastrar un único archivo local `.mp4`, `.mov`, `.mxf`,
  ...
  - Sin track seleccionado, o si no entra, abre `ProjectMediaPreviewDialog` con
    ...
    aunque haya nombres duplicados: el destino se elige haciendo click sobre el
